...
template<class Pred>
struct FeatureCollector : public Filter::Feature<FeatureCollector<Pred>, Pred >
{
...
TEST(Feature_Filter_Test, DropsAllWhenPredicateFalse)
{
	...
	struct PredAlwaysFalse { int calls = 0; bool operator()(const Feature&){ ++calls; return false; } };

	FeatureCollector<PredAlwaysFalse> filter;
