...
TYPED_TEST_P(Merge_Test, Nominal)
{
	...
	Merge_Exemple::Classifier_Storage dcel = Merge_Exemple::From_Vertex(TypeParam::vertex, TypeParam::edges);
	for (auto object : TypeParam::moves)
	{
		auto& vertex_to_move = dcel.vertices[object.vertex_id];
		dcel.Move(vertex_to_move, object.new_x, object.new_y);
	}

	EXPECT_TRUE(!!true);

	for (auto&& [vertex, expected_vertex] : O::Zip(dcel.vertices, TypeParam::expected_vertex))
	{
